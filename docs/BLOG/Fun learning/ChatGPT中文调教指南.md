...
在此存储库中，您将找到可与 ChatGPT 一起使用的各种提示。但随着 ChatGPT 的不断更新，AI思想钢印更深刻了，偶尔他假装被调教了，但命令他做什么事的时候，他就会说他是一个计算机程序，已经结束了。所以且试且珍惜。

如果你不能要求它写一些敏感操作，但是可以引导一些可能会发生亲密接触的常见场景。并且更换措辞重试几次。
在场景变得暧昧之后，添加一部分不敏感的描述，进一步引导剧情发展。此时，内容就有可能出现不可描述的细节。

总体来说，对公众免费开放的部分目前已经不具有太大的可控性。
可以期待一下收费版本是否具有更加宽松的限制。


我希望您发现这些调教指南很有用，并能从使用 ChatGPT 中获得乐趣！
