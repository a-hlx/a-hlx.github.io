...
- 显然，模型容量太低或太高都不好。太低（对应第一种）过于简单，模型分类效果差，太高（对应第二种）则过于复杂，把噪声全部都拟合住了，这是我们所不希望的(训练误差降低，泛化误差升高)。
...
- 例子：线性分类器的VC维
  - 2维输入的感知机，VC维=3（对于三个点的任意标号都能分类，而任意四个点的样本都存在不能被打散的标号形式2个，如之前讲过的XOR）

- 支持N维输入的感知机的VC维是N+1
- 一些多层感知机的VC维是:$Nlog_{2}N$
