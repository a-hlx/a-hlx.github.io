...
我们的优化目标仍然是$min\space\ell(\bold{w},b)$，
只是额外对$bold{w}$
添加一个限制条件
$||\bold{w}||^2\leqslant\theta)$，
即权重的各项平方和小于一个特定的常数$\theta$。那么设定一个较小的$\theta$就会使得$\bold{w}$中每个元素的值都不会太大。
...
![](https://cdn.jsdelivr.net/gh/HLIX1/pic/202210091446656.png)
...
将上式结果带入时间t更新参数公式
$\bold{w}_{t+1}=\bold{w}_{t}-\eta\frac{\partial}{{\partial{\bold{w}_{t}}}}$
整理可得
